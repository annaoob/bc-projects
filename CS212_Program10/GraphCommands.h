...
// is successful, otherwise addition has failed.
		...
		if (newVertex) {
			if (!shouldSucceed) {
				testFeedback << "FAIL: AddVertex(\"" << label
						<< "\") returned a ";
				testFeedback << "non-null pointer, but should have returned a ";
				testFeedback
						<< "null pointer due to the label already being in ";
		...
		if (shouldSucceed) {
			...
			testFeedback
					<< "null pointer, but should have returned a non-null ";
			testFeedback << "pointer" << std::endl;
			return false;
		}
		testFeedback << "PASS: AddVertex(\"" << label
				<< "\") returned nullptr ";
...
// If the vertex shouldn't exist, then print a failure message
			if (!shouldExist) {
				testFeedback << "FAIL: GetVertex(\"" << label
						<< "\") returned a ";
				testFeedback << "non-null pointer, but should have returned a ";
...
// Call HasEdge() to get the actual return value
		...
		if (actual != expected) {
			...
			testFeedback << (expected ? "true" : "false")
					<< " for an edge from ";
...
// Find fromVertex
		Vertex *fromVertex = graph.GetVertex(fromLabel);
		if (!fromVertex) {
			testFeedback << "FAIL: GetVertex(\"" << fromLabel
					<< "\") returned ";
			testFeedback << "nullptr for a vertex that should exist"
					<< std::endl;
			return false;
		}
