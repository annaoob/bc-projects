...
public:
   ...
   Node<T>* RotateLeft(Node<T>* node) {
      ...
      node->right->SetChild(Node<T>::LEFTCHILD, node);

      // Step 3 - reattach rightLeftChild as the right child of node.
      node->SetChild(Node<T>::RIGHTCHILD, rightLeftChild);
   ...
   Node<T>* RotateRight(Node<T>* node) {
      ...
      node->left->SetChild(Node<T>::RIGHTCHILD, node);

      // Step 3 - reattach leftRightChild as the right child of node.
      node->SetChild(Node<T>::LEFTCHILD, leftRightChild);
